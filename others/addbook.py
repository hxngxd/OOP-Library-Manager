...
for index, row in authors_df.iterrows():
    ...
    date_of_birth = row['Date of Birth'].strftime('%Y-%m-%d') if not pd.isnull(row['Date of Birth']) else 'NULL'
    biography = row['Biography'].replace("'", "''") if not pd.isnull(row['Biography']) else ''
    date_of_death = row['Date of Death'].strftime('%Y-%m-%d') if not pd.isnull(row['Date of Death']) else 'NULL'
...
for index, row in books_df.iterrows():
    ...
    number_of_pages = int(row['Number of Pages']) if not pd.isnull(row['Number of Pages']) else 'NULL'
    publication_year = int(row['Publication Year']) if not pd.isnull(row['Publication Year']) else 'NULL'
    description = row['Description'].replace("'", "''")
    ...
    for author in authors:
        ...
        if author_id:
            book_author_sql = f"INSERT INTO bookAuthor (bookId, authorId) VALUES ({book_id}, {author_id});"
    ...
    for genre in genres:
        genre = genre.strip()
        book_genre_sql = f"INSERT INTO bookGenre (bookId, genreId) VALUES ({book_id}, (SELECT id FROM genre WHERE name='{genre}'));"
